...
:build_cmd
echo rem Build this Project with many dlls into a Distr folder
echo rem dotnet publish %ProjectDirName%\%ProjectFileName% -o Distr
echo.
echo rem Build this Project as a single-file app when NET Desktop runtime required
echo dotnet publish %ProjectDirName%\%ProjectFileName% -o Distr -r win-x64 -p:PublishSingleFile=true --self-contained false
echo.
echo rem Build this Project as a single-file app when no runtime required
